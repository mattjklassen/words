# this version will do difference of 5 or more between n and m, 
# n < m to generate n-m-edges.txt or examples.

# We loop through m-nodes beta and build lists of leaves and nodes by
# using the tree of alpha strings and sub-alphas.  We obtain lists
# of all nodes of lengths n through m-1 which are subsets of the
# m-node beta. We then check for edges from the list of n-nodes to
# any of the other nodes of length n+1 to m-1, eliminating the n-node
# when such edge is found.  The remaining n-nodes need an n->m edge.
